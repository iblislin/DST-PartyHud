...
asrt.are = {
  same = function(a, b)
    if not deepeq(a, b) then
      error("are.same failed", 2)
    end
  end,
  equal = function(a, b)
    if a ~= b then
      error("are.equal failed: " .. show(a) .. " ~= " .. show(b), 2)
    end
  end,
}
asrt.is_nil = function(v)
  if v ~= nil then
    error("is_nil failed: got " .. show(v), 2)
  end
end
asrt.is_true = function(v)
  if v ~= true then
    error("is_true failed: got " .. show(v), 2)
  end
end
asrt.is_false = function(v)
  if v ~= false then
    error("is_false failed: got " .. show(v), 2)
  end
end
asrt.is_truthy = function(v)
  if not v then
    error("is_truthy failed", 2)
  end
end
asrt.is_string = function(v)
  if type(v) ~= "string" then
    error("is_string failed", 2)
  end
end
asrt.has_error = function(fn)
  if pcall(fn) then
    error("has_error failed: expected an error", 2)
  end
end
_G.assert = asrt
